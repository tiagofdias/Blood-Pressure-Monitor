...
export async function exportToExcel(readings: BPReading[], userName: string = 'Patient') {
  ...
  summarySheet.columns = [
    { width: 30 }, // A
    { width: 25 }, // B
    { width: 15 }, // C - Numeric values
    { width: 30 }, // D - Target Range
    { width: 25 }  // E - Status
  ];
  ...
  const patientInfo = [
    ...
    ['Total Readings:', readings.length, '', ''], // Keep as number
  ...
  // Stats table header
  const statsHeaders = ['📊 Metric', '📈 Value', '📊 Numeric', '🎯 Target Range', '⚕️ Status'];
  ...
  const statsData = [
    ['Average Systolic BP', `${avgSys} mmHg`, avgSys, '< 120 mmHg', avgSys < 120 ? '✅ NORMAL' : avgSys < 130 ? '⚠️ ELEVATED' : avgSys < 140 ? '⚠️ STAGE 1 HIGH' : avgSys < 180 ? '❌ STAGE 2 HIGH' : '🚨 CRISIS'],
    ['Average Diastolic BP', `${avgDia} mmHg`, avgDia, '< 80 mmHg', avgDia < 80 ? '✅ NORMAL' : avgDia < 90 ? '⚠️ STAGE 1 HIGH' : avgDia < 120 ? '❌ STAGE 2 HIGH' : '🚨 CRISIS'],
  ...
  if (heartRateReadings.length > 0) {
    ...
    statsData.push(['Average Heart Rate', `${avgHR} bpm`, avgHR, '60-100 bpm', hrStatus]);
  }
  ...
  statsData.forEach((row, rowIndex) => {
    ...
    row.forEach((cell, colIndex) => {
      ...
      if (colIndex === 4 && cell && typeof cell === 'string') {
      ...
      } else if (colIndex === 2 && typeof cell === 'number') {
  ...
  Object.entries(categoryCount)
    ...
    .forEach(([category, count], index) => {
      ...
      const data = [
        category,
        count, // Keep as number
        percentage, // Keep as number
      ...
      data.forEach((cellValue, colIndex) => {
        ...
        } else if (colIndex === 1 && typeof cellValue === 'number') {
          // Count column - format as integer
          cell.style = {
            font: { size: 10, bold: true },
            alignment: { horizontal: 'center', vertical: 'middle' },
        ...
        } else if (colIndex === 2 && typeof cellValue === 'number') {
  ...
  sortedReadings.forEach((reading, index) => {
    ...
    const rowData = [
      `${dayOfWeek} ${dateStr}`,
      timeStr,
      reading.systolic, // Keep as number
      reading.diastolic, // Keep as number
      reading.heartRate || null, // Keep as number or null
      reading.category,
      '' // Notes
    ];
    ...
    rowData.forEach((cellValue, colIndex) => {
      ...
      // Handle null heart rate values
      if (colIndex === 4 && cellValue === null) {
        cell.value = 'N/A';
      } else {
        cell.value = cellValue;
      }
      ...
      // Special formatting for numeric columns (systolic, diastolic, heart rate)
      if ((colIndex === 2 || colIndex === 3) && typeof cellValue === 'number') {
        cellStyle.numFmt = '0'; // Format as integer
        cellStyle.font = { size: 10, bold: true };
        
        // Special styling for high readings
        if (isHighReading) {
          cellStyle.font.color = { argb: colors.stage2Red };
          cellStyle.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: colors.lightRed } };
        }
      } else if (colIndex === 4 && typeof cellValue === 'number') {
        cellStyle.numFmt = '0'; // Format heart rate as integer
        cellStyle.font = { size: 10, bold: true };
      }
